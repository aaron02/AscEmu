...
The active World V2 profile is Midnight 12.1.0 build 69814. The current profile defines:
...
`SMSG_ENTER_ENCRYPTED_MODE` is serialized as a 64-byte Ed25519ctx signature followed by the enabled bit.

The exact build-auth key for build 69814 is intentionally not hardcoded until independently verified. Unsupported or unverified builds are rejected rather than falling back to another World V2 profile.
