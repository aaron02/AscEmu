...
namespace AscEmu::Battlenet
{
    ...
    namespace
    {
        ...
        {
            static const uint32_t epoch = static_cast<uint32_t>(
        ...
        std::string buildClientInstanceId(
            uint32_t serverLabel,
            uint32_t serverEpoch,
            uint32_t clientLabel,
        ...
        std::vector<uint8_t> makeGameUtilitiesStringVariantList(
        ...
        std::vector<uint8_t> compressGameUtilitiesJson(const std::string& json)
        {
            ...
            const int zResult = compress(
                reinterpret_cast<Bytef*>(result.data() + 4u),
                &compressedSize,
                reinterpret_cast<const Bytef*>(json.c_str()),
                sourceSize);
        ...
        void appendGameUtilitiesBlobAttribute(
            std::vector<uint8_t>& response,
        ...
        bool extractRealmListIdentityGameAccountId(
            const uint8_t* payload,
            size_t payloadSize,
            uint32_t& gameAccountId)
        {
            gameAccountId = 0;
        ...
        bool readGameUtilitiesUintAttribute(
            const uint8_t* payload,
            size_t payloadSize,
            const std::string& wantedName,
            uint64_t& value)
        ...
        std::vector<uint8_t> makeRealmListResponse(uint32_t clientBuild, uint32_t accountId)
        {
            ...
            if (sBNetLogonSQL)
            {
                if (auto result = sBNetLogonSQL->query(
                        "SELECT id, status FROM realms ORDER BY id"))
                {
                    do
                    {
            ...
            for (const RealmRow& realm : realms)
            {
                ...
                const uint32_t address = makeRealmAddress(
                    static_cast<uint8_t>(Protocol::WoW::EuropeRegion), 1u,
            ...
            if (accountId != 0u && sBNetCharacterSQL)
            {
                for (const RealmRow& realm : realms)
                {
                    ...
                    if (auto countResult = sBNetCharacterSQL->query(
                            "SELECT COUNT(*) FROM characters WHERE acct = %u",
                            accountId))
                    {
        ...
        std::vector<uint8_t> makeGameUtilitiesBlobAttribute(
            const std::string& name,
            ...
            size_t payloadSize)
        {
            ...
            for (size_t i = 0; i + prefixLength <= payloadSize; ++i)
            {
                ...
                {
                    return std::string(
                        reinterpret_cast<const char*>(payload + i),
                        end - i);
                }
            }
        ...
        bool parseSingleStringFieldOne(
            const uint8_t* payload,
            size_t payloadSize,
            std::string& value)
        {
        ...
        AuthenticationV2LogonRequestDiagnostic parseAuthenticationV2LogonRequest(
            const uint8_t* data,
            size_t size)
        {
    ...
    bool BNetSocket::processRpcFrames()
    {
        while (m_plainTextBuffer.size() >= 2)
        {
            ...
            m_plainTextBuffer.erase(
                m_plainTextBuffer.begin(),
                m_plainTextBuffer.begin() + static_cast<std::ptrdiff_t>(frameSize)
            );
    ...
    bool BNetSocket::dispatchRpcRequest(
        ...
        size_t payloadSize)
    {
        ...
        {
            if (methodId == Protocol::AccountServiceV2::GetAccountInfo ||
                methodId == Protocol::AccountServiceV2::GetRestriction ||
                methodId == Protocol::AccountServiceV2::GetGameAccountLinks ||
                methodId == Protocol::AccountServiceV2::GetGameAccountInfo ||
                methodId == Protocol::AccountServiceV2::GetGameAccountRestriction)
            {
        ...
        else if (serviceHash == Protocol::GameUtilitiesService::Hash)
        {
            if (methodId == Protocol::GameUtilitiesService::ProcessClientRequest ||
                methodId == Protocol::GameUtilitiesService::GetAllValuesForAttribute)
            {
    ...
    void BNetSocket::logUnhandledRpc(
        uint32_t serviceHash,
        uint32_t methodId,
        uint32_t token,
        const uint8_t* payload,
    ...
    bool BNetSocket::handleConnectionConnect(uint32_t token, const uint8_t* payload, size_t payloadSize)
    {
        ...
        const uint64_t serverTime = static_cast<uint64_t>(
        ...
        const uint32_t clientLabel = static_cast<uint32_t>(
            m_connectionId & std::numeric_limits<uint32_t>::max()
        );
        const uint32_t clientEpoch = static_cast<uint32_t>(
            std::chrono::duration_cast<std::chrono::seconds>(now.time_since_epoch()).count()
        );

        const std::string clientInstanceId = buildClientInstanceId(
            serverLabel,
            serverEpoch,
            clientLabel,
            clientEpoch
    ...
    bool BNetSocket::handleConnectionKeepAlive(
        uint32_t token,
        const uint8_t* /*payload*/,
        size_t payloadSize)
    {
    ...
    bool BNetSocket::handleConnectionRequestDisconnect(
        uint32_t token,
        const uint8_t* payload,
        size_t payloadSize)
    {
        ...
        if (!sendRpcRequest(
                Protocol::ConnectionService::Hash,
                Protocol::ConnectionService::ForceDisconnect,
    ...
    bool BNetSocket::handleAuthenticationV2Logon(
        uint32_t token,
        const uint8_t* payload,
        size_t payloadSize)
    {
    ...
    bool BNetSocket::handleAuthenticationV2VerifyWebCredentials(
        uint32_t token,
        const uint8_t* payload,
        size_t payloadSize)
    {
    ...
    bool BNetSocket::sendLogonComplete(
        ...
        const std::string& loginTicket)
    {
        ...
        auto result = sBNetLogonSQL->query(
            "SELECT ba.id, ba.email, ba.battle_tag, ba.country, a.id, a.acc_name "
            "FROM battlenet_accounts ba "
            "INNER JOIN battlenet_game_accounts bga ON bga.battlenet_account_id = ba.id "
            "INNER JOIN accounts a ON a.id = bga.game_account_id "
            "WHERE UPPER(ba.email) = UPPER('%s') "
            "ORDER BY a.id",
            escapedLogin.c_str());
        ...
        return sendRpcRequest(
            ...
            Protocol::AuthenticationListenerV2::OnLogonComplete,
            serverToken,
            notification
        );
    }

    bool BNetSocket::handleGameUtilitiesRequest(
        uint32_t methodId,
        uint32_t token,
        const uint8_t* payload,
        size_t payloadSize)
    ...
    bool BNetSocket::handleGameUtilitiesGetAllValues(
        uint32_t token,
    ...
    bool BNetSocket::handleRealmListTicketRequest(
        uint32_t token,
        ...
        size_t payloadSize)
    {
        ...
        const auto linkedAccount = std::find_if(
            m_linkedGameAccounts.begin(),
            m_linkedGameAccounts.end(),
            [requestedGameAccountId](const LinkedGameAccount& account)
            {
                return account.id == requestedGameAccountId;
            });
        ...
        const std::vector<uint8_t> response = makeGameUtilitiesBlobAttribute(
    ...
    bool BNetSocket::handleAccountServiceV2Request(
        uint32_t methodId,
        uint32_t token,
        const uint8_t* payload,
        size_t payloadSize)
        ...
        switch (methodId)
        {
            ...
            case Protocol::AccountServiceV2::GetGameAccountInfo:
            {
                ...
                const auto linked = std::find_if(
                    m_linkedGameAccounts.begin(), m_linkedGameAccounts.end(),
                    [accountId](const LinkedGameAccount& account) { return account.id == accountId; });

                if (linked == m_linkedGameAccounts.end() ||
    ...
    bool BNetSocket::sendExternalChallenge()
    {
        ...
        return sendRpcRequest(
            Protocol::AuthenticationListenerV2::Hash,
            Protocol::AuthenticationListenerV2::OnExternalChallenge,
            serverToken,
            challenge
        );
    }

    bool BNetSocket::sendRpcRequest(
        uint32_t serviceHash,
        uint32_t methodId,
        uint32_t token,
    ...
    {
        const auto connectedFor = std::chrono::duration_cast<std::chrono::milliseconds>(
