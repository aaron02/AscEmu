...
namespace AscEmu::Battlenet
{
    ...
    {
        std::lock_guard lock(m_mutex);
        std::erase_if(m_realms, [socket](const auto& entry)
        {
            return entry.second == socket;
        });
    }
    ...
    void BattleNetCommServerSocket::onRead()
    {
        while (true)
        {
            if (!m_headerReady)
            {
                ...
                {
                    sLogger.failure(
                        "BattleNetComm: invalid frame magic from {}:{} (first bytes {:02X} {:02X} {:02X} {:02X})",
                        getRemoteIp(), getRemotePort(), headerBytes[0], headerBytes[1], headerBytes[2], headerBytes[3]);
                    disconnect();
                    return;
                }
                ...
                {
                    sLogger.failure("BattleNetComm: unsupported frame version {} from {}:{} (expected {})",
                        header.version, getRemoteIp(), getRemotePort(), BattlenetComm::FrameVersion);
                    disconnect();
                    return;
                }
                ...
                {
                    sLogger.failure("BattleNetComm: packet too large from {}:{} ({} bytes, max {})",
                        getRemoteIp(), getRemotePort(), header.payloadSize, BattlenetComm::MaxPacketSize);
                    disconnect();
                    return;
                }
    ...
    bool BattleNetCommServerSocket::sendPacket(WorldPacket& packet)
    {
        ...
        {
            sLogger.failure("BattleNetComm: refusing to send oversized packet opcode={} size={} max={}",
                packet.getOpcode(), packet.size(), BattlenetComm::MaxPacketSize);
            return false;
        }

        std::array<uint8_t, BattlenetComm::FrameHeaderSize> header{};
        BattlenetComm::encodeFrameHeader(
            header,
    ...
    void BattleNetCommServerSocket::handleRegisterRealm(WorldPacket& packet)
    {
        ...
        {
            sLogger.failure("BattleNetComm: malformed realm registration from {}:{} (payload={} bytes, consumed={} bytes)",
                getRemoteIp(), getRemotePort(), packet.size(), packet.rpos());
            disconnect();
            return;
        }
        ...
        sLogger.info("BattleNetComm: registered realm {} ('{}') from {}:{}",
            realmId, realmName, getRemoteIp(), getRemotePort());
    }
