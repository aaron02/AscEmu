...
namespace AscEmu::Battlenet
{
    namespace
    {
        ...
        void storePendingSrpChallenge(
            const std::string& login,
            const AscEmu::Cryptography::BNetSrpV1::ServerChallenge& challenge)
        {
    ...
    void WebAuthSocket::onConnect()
    {
        ...
        {
            sLogger.failure(
                "BNet WebAuth: connection #{} could not initialize TLS",
                m_connectionId
            );
            disconnect();
            return;
        ...
        sLogger.info(
            "BNet WebAuth: connection #{} TLS handshake started",
            m_connectionId
        );
    }
    ...
    void WebAuthSocket::onRead()
    {
        ...
        {
            sLogger.failure(
                "BNet WebAuth: connection #{} failed to read {} byte(s)",
                m_connectionId,
                available
            );
            disconnect();
        ...
        {
            sLogger.failure(
                "BNet WebAuth: connection #{} failed to pass TLS input to OpenSSL",
                m_connectionId
            );
            disconnect();
            return;
    ...
    bool WebAuthSocket::processTls()
    {
        if (!m_tlsHandshakeComplete)
        {
            ...
            if (result == 1)
            {
                ...
                sLogger.info(
                    "BNet WebAuth: connection #{} TLS handshake complete "
                    "(version: {}, cipher: {}, SNI: '{}')",
                    m_connectionId,
                    SSL_get_version(m_ssl),
                    SSL_get_cipher_name(m_ssl),
                    serverName != nullptr ? serverName : "<none>"
                );
            }
            else
            {
    ...
    bool WebAuthSocket::processHttpData(const uint8_t* data, size_t size)
    {
        ...
        {
            sLogger.failure(
                "BNet WebAuth: connection #{} HTTP request exceeded {} byte(s)",
                m_connectionId,
        ...
        while (lineStart < headerEnd)
        {
            ...
            if (lowerLine.rfind(contentLengthName, 0) == 0)
            {
                ...
                {
                    sLogger.failure(
                        "BNet WebAuth: connection #{} invalid Content-Length '{}',",
                        m_connectionId,
                        value
                    );
                    return false;
        ...
        {
            sLogger.failure(
                "BNet WebAuth: connection #{} HTTP request requires {} byte(s), limit is {}",
                m_connectionId,
                headersSize + contentLength,
                MAX_HTTP_REQUEST_SIZE
            );
            return false;
        }
        ...
        if (requestLine.rfind("POST /bnetserver/login/ ", 0) == 0)
        {
            ...
            {
                sLogger.info(
                    "BNet WebAuth: connection #{} POST /bnetserver/login/ is not a complete SRP proof",
                    m_connectionId
                );
            ...
            {
                sLogger.info(
                    "BNet WebAuth: connection #{} has no pending SRP challenge for '{}'",
                    m_connectionId,
                    login
                );
                return sendJsonResponse(
            ...
            if (!AscEmu::Cryptography::BNetSrpV1::verifyClientEvidence(
                    challenge,
                    publicA,
                    clientM1,
                    serverM2,
            ...
            {
                sLogger.info(
                    "BNet WebAuth: connection #{} SRP client evidence rejected for '{}'",
                    m_connectionId,
                    login
                );
                sLogger.info(
                ...
                sLogger.info(
                    ...
                    evidenceDiagnostics.littleEndianUExpectedM1
                );
                return sendJsonResponse(
                    "{\"authentication_state\":\"DONE\"}");
            }
            ...
            {
                sLogger.failure(
                    "BNet WebAuth: connection #{} could not generate login ticket",
                    m_connectionId
                );
        ...
        if (requestLine.rfind("POST /bnetserver/login/srp/ ", 0) == 0)
        {
            ...
            {
                sLogger.failure(
                    "BNet WebAuth: connection #{} SRP challenge request has no account_name",
                    m_connectionId
                );
                return sendJsonResponse(
                    "{\"authentication_state\":\"DONE\",\"error_code\":\"UNKNOWN_ACCOUNT\"}");
            }

            if (!sBNetLogonSQL)
            {
                sLogger.failure(
                    "BNet WebAuth: connection #{} logon database is unavailable",
                    m_connectionId
                );
                return sendJsonResponse("{}", "500 Internal Server Error");
            }
            ...
            auto result = sBNetLogonSQL->query(
                "SELECT email, srp_version, srp_salt, srp_verifier "
                "FROM battlenet_accounts WHERE UPPER(email) = UPPER('%s') LIMIT 1",
                escapedLogin.c_str());

            if (!result)
            {
                sLogger.info(
                    "BNet WebAuth: connection #{} SRP account '{}' not found",
                    m_connectionId,
                    login
                );
                return sendJsonResponse(
            ...
            {
                sLogger.info(
                    "BNet WebAuth: connection #{} account '{}' has no Battle.net SRP v1 credentials; "
                    "reset the account password once after applying the Stage 13 SQL update",
                    m_connectionId,
                    accountEmail
                );
                return sendJsonResponse(
                    "{\"authentication_state\":\"DONE\",\"error_code\":\"SRP_CREDENTIALS_MISSING\"}");
            }

            AscEmu::Cryptography::BNetSrpV1::ServerChallenge challenge;
            if (!AscEmu::Cryptography::BNetSrpV1::createServerChallenge(
                    srpIdentity,
                    saltHex,
                    verifierHex,
                    challenge))
            {
                sLogger.failure(
                    "BNet WebAuth: connection #{} failed to build SRP v1 challenge for account '{}'",
                    m_connectionId,
                    accountEmail
                );
                return sendJsonResponse("{}", "500 Internal Server Error");
            ...
            sLogger.info(
                "BNet WebAuth: connection #{} POST /bnetserver/login/srp/ -> "
                "SRP v1 challenge [legacy little-endian] for account '{}' using identity '{}' "
                "(username={}, salt={} byte(s), B={} byte(s)); ephemeral proof state cached",
                m_connectionId,
                accountEmail,
                srpIdentity,
                challenge.username,
                challenge.salt.size() / 2,
                challenge.publicB.size() / 2
            );
        ...
        sLogger.failure(
            "BNet WebAuth: connection #{} unsupported HTTP request '{}',",
            m_connectionId,
            requestLine
        );
    ...
    bool WebAuthSocket::sendLoginFormResponse()
    {
        ...
        sLogger.info(
            "BNet WebAuth: connection #{} GET /bnetserver/login/ -> "
            "HTTP 200 LOGIN_FORM with SRP URL ({} byte(s))",
            m_connectionId,
            sizeof(body) - 1
        );

        if (!writeTlsPlainText(
    ...
    bool WebAuthSocket::sendJsonResponse(
    ...
    void WebAuthSocket::logTlsError(const char* operation, int result) const
    {
        ...
        {
            sLogger.failure(
                "BNet WebAuth: connection #{} TLS {} failed (SSL error {})",
                m_connectionId,
                operation,
                sslError
            );
    ...
    {
        sLogger.info(
            "BNet WebAuth: connection #{} disconnected",
            m_connectionId
        );
