...
void NazanAI::OnSpellHitTarget(Object* target, SpellInfo const* info)
{
    ...
    {
        if (Creature* fire = summonCreature(NPC_LIQUID_FIRE, target->GetPositionX(), target->GetPositionY(), target->GetPositionZ(), target->GetOrientation(), 30000))
...
void VazrudenTheHeraldAI::summonAdds()
{
    if (!summoned)
    {
        if (Creature* Vazruden = summonCreature(NPC_VAZRUDEN, VazrudenMiddle[0], VazrudenMiddle[1], VazrudenMiddle[2], 0, 100 * 60 * 1000))
        {
        ...
        if (Creature* Nazan = summonCreature(NPC_NAZAN, VazrudenMiddle[0], VazrudenMiddle[1], VazrudenMiddle[2], 0, 100 * 60 * 1000))
        {
