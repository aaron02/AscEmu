...
namespace AscEmu::BattlenetComm
{
    ...
    void BattleNetCommClientSocket::onConnect()
    {
        ...
        sLogger.info("BattleNetCommClient: connected to {}:{}; registering realm {} ('{}')",
            worldConfig.battleNetComm.host,
            worldConfig.battleNetComm.port,
            worldConfig.battleNetComm.realmId,
            worldConfig.battleNetComm.realmName);
    }
    ...
    void BattleNetCommClientSocket::onRead()
    {
        while (true)
        {
            if (!m_headerReady)
            {
                ...
                {
                    sLogger.failure(
                        "BattleNetCommClient: invalid frame magic (first bytes {:02X} {:02X} {:02X} {:02X})",
                ...
                {
                    sLogger.failure("BattleNetCommClient: unsupported frame version {} (expected {})",
                ...
                {
                    sLogger.failure("BattleNetCommClient: packet too large ({} bytes, max {})",
                        header.payloadSize, MaxPacketSize);
                    disconnect();
                    return;
                }
    ...
    bool BattleNetCommClientSocket::sendPacket(WorldPacket& packet)
    {
        ...
        {
            sLogger.failure("BattleNetCommClient: refusing to send oversized packet opcode={} size={} max={}",
                packet.getOpcode(), packet.size(), MaxPacketSize);
            return false;
        }
    ...
    void BattleNetCommClientSocket::handleRegisterResult(WorldPacket& packet)
    {
        ...
        {
            sLogger.failure("BattleNetCommClient: malformed realm registration result (payload={} bytes, consumed={} bytes)",
                packet.size(), packet.rpos());
            disconnect();
            return;
        }
    ...
    void BattleNetCommClientSocket::handlePendingWorldSession(WorldPacket& packet)
    {
        ...
        if (packet.hadReadFailure() || packet.rpos() != packet.size())
        {
            sLogger.failure("BattleNetCommClient: malformed pending world session payload (payload={} bytes, consumed={} bytes)",
                packet.size(), packet.rpos());
            disconnect();
            return;
        }
    ...
    {
        if (!worldConfig.battleNetComm.enabled)
            return;

        m_running.store(true);
        threadPool.addDedicatedThread(
            "BattleNetCommWatcher",
            [this](AscEmu::Threading::AEThread& thread)
            {
                run(thread);
            });
    }
    ...
    void BattleNetCommClientManager::run(AscEmu::Threading::AEThread& thread)
    {
        while (m_running.load() && !thread.isKilled())
        {
            ...
            if (!connected)
            {
                sLogger.info("BattleNetCommClient: attempting connection to {}:{}",
                    worldConfig.battleNetComm.host, worldConfig.battleNetComm.port);

                if (auto* socket = ConnectTCPSocket<BattleNetCommClientSocket>(
                        worldConfig.battleNetComm.host.c_str(),
                        static_cast<u_short>(worldConfig.battleNetComm.port)); socket == nullptr)
                {
    ...
    void BattleNetCommClientManager::storePendingSession(PendingWorldSession session)
    {
        ...
        sLogger.info("BattleNetCommClient: received pending world session ticket_bytes={} (pending={})",
            ticket.size(), pendingCount);
    }

    bool BattleNetCommClientManager::getPendingSession(
    ...
    void BattleNetCommClientManager::cleanupExpiredSessions()
    {
        ...
        std::lock_guard lock(m_pendingMutex);
        std::erase_if(m_pendingSessions, [now](const auto& entry)
        {
