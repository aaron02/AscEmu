...
namespace world 
{
    ...
    template <class Map>
    static auto findByLowGuid(const Map& map, uint32_t low)
    {
        for (const auto& [_, obj] : map)
        {
            if (obj && obj->GetNewGUID().getGuidLow() == low)
                return obj;
        }
    ...
    Creature* WorldObjectRegistry::getCreature(const uint32_t low) const
    {
        std::shared_lock lk(mtx_);
        return findByLowGuid(creatures_, low);
    }
    ...
    GameObject* WorldObjectRegistry::getGameObject(const uint32_t low) const
    {
        std::shared_lock lk(mtx_);
        return findByLowGuid(gameobjects_, low);
    }
    ...
    Transporter* WorldObjectRegistry::getTransporter(const uint32_t low) const
    {
        std::shared_lock lk(mtx_);
        return findByLowGuid(transporters_, low);
    }
    ...
    DynamicObject* WorldObjectRegistry::getDynamicObject(const uint32_t low) const
    {
        std::shared_lock lk(mtx_);
        return findByLowGuid(dynamics_, low);
    }
    ...
    Player* WorldObjectRegistry::getPlayer(const uint32_t low) const
    {
        std::shared_lock lk(mtx_);
        return findByLowGuid(players_, low);
    }
    ...
    Pet* WorldObjectRegistry::getPet(const uint32_t low) const
    {
        std::shared_lock lk(mtx_);
        return findByLowGuid(pets_, low);
    }
    ...
    Corpse* WorldObjectRegistry::getCorpse(const uint32_t low) const
    {
        std::shared_lock lk(mtx_);
        return findByLowGuid(corpses_, low);
    }
    ...
    Corpse* WorldObjectRegistry::getCorpseByOwner(uint32_t ownerLowGuid) const
    {
        ...
        for (const auto& [_, corpse] : corpses_)
        {
            if (!corpse)
                continue;
            ...
            if (owner.getGuidLowPart() != ownerLowGuid)
                continue;

            // If duplicate corpses exist because of a previous crash, prefer the
            // newest body corpse. Older duplicate/bone corpses must not block
            // reclaim/resurrection logic.
    ...
    Summary WorldObjectRegistry::counts() const
    {
        ...
        s.transporters = transporters_.size();
