    ...
    _terrain(std::make_unique<TerrainHolder>(id)), m_unloadTimer(expiry), m_baseMap(baseMap)
{
    ...
    cfg.cellUnloadDelay = std::chrono::minutes(1);

    spatialIndex_ = std::make_unique<visibility::SpatialIndex>(/*pool cap*/ 32767);
...
void WorldMap::update(uint32_t t_diff)
{
    ...
    if (getBaseMap()->getMapId() != 1 && getBaseMap()->getMapId() != 0)
        return;
...
void WorldMap::hookVisibilityEvents()
{
    visibilitySystem_->onGridActivated([this](int gid)
        {
            sLogger.info("Grid Activated {} ", gid);
    ...
    visibilitySystem_->onGridDeactivated([this](int gid)
        {
            sLogger.info("Grid Deactivated {}", gid);
        });

    visibilitySystem_->onGridUnload([this](int gid)
        {
            sLogger.info("Grid Unloaded {} ", gid);
...
void WorldMap::onPlayerEnter(Player* plr)
{
    ...
    factory_->attachToWorld(plr, plr->GetPosition(), /*asActivator=*/true, /*asViewer=*/true);

    auto p = plr->GetPosition();
    auto [gx, gy] = visibility::worldToGrid(p);
    auto [cx, cy] = visibility::worldToLocal(p);
    sLogger.info("Player entered at pos=({:.1f},{:.1f}) grid=({},{}) cell=({},{})", p.x, p.y, gx, gy, cx, cy);
}
...
void WorldMap::onObjectMoved(Object* obj)
{
    ...
    // Debug
    if (obj->ToPlayer() && false)
    {
        auto p = obj->GetPosition();
        auto [gx, gy] = visibility::worldToGrid(p);
        auto [cx, cy] = visibility::worldToLocal(p);

        constexpr int Rcells = 2;

        std::vector<WoWGuid> vPlayers, vCreatures, vPets, vGos, vDyns, vCorpses;
        spatialIndex_->collectGuidsInCellsAroundPos<Player>(p, Rcells, 0, vPlayers);
        spatialIndex_->collectGuidsInCellsAroundPos<Creature>(p, Rcells, 0, vCreatures);
        spatialIndex_->collectGuidsInCellsAroundPos<Pet>(p, Rcells, 0, vPets);
        spatialIndex_->collectGuidsInCellsAroundPos<GameObject>(p, Rcells, 0, vGos);
        spatialIndex_->collectGuidsInCellsAroundPos<DynamicObject>(p, Rcells, 0, vDyns);
        spatialIndex_->collectGuidsInCellsAroundPos<Corpse>(p, Rcells, 0, vCorpses);

        const size_t total =
            vPlayers.size() + vCreatures.size() + vPets.size() +
            vGos.size() + vDyns.size() + vCorpses.size();

        sLogger.info(
            "Player at pos=({:.1f},{:.1f}) grid=({},{}) cell=({},{}) -> in-range: total={} (players={}, creatures={}, pets={}, gos={}, dynos={}, corpses={})",
            p.x, p.y, gx, gy, cx, cy,
            total, vPlayers.size(), vCreatures.size(), vPets.size(),
            vGos.size(), vDyns.size(), vCorpses.size()
        );
    }
}
...
{
    Player* player = getPlayerByGuid(viewerGuid);
    if (!player)
        return false;
    ...
    // If the object already left the player's visibility again while queued, do not create it.
    ...
    initialCreateValueUpdateSuppress_[viewerGuid.getRawGuid()].insert(objectGuid.getRawGuid());

    // The create block already contains the current values for this viewer.
...
bool WorldMap::applyQueuedVisibilityHidden(const WoWGuid& viewerGuid, const WoWGuid& objectGuid)
{
    ...
    if (auto it = initialCreateValueUpdateSuppress_.find(viewerGuid.getRawGuid()); it != initialCreateValueUpdateSuppress_.end())
    {
...
void WorldMap::changeFarsightLocation(Player* plr, DynamicObject* farsight)
{
    ...
    if (farsight == nullptr)
    {
        for (auto it = farSet.begin(); it != farSet.end(); ++it)
        {
            ...
            if (plr->isVisibleObject(obj->getGuid()) && !plr->canSee(obj))
    ...
    auto consider = [&](Object* obj)
        {
            ...
            // build a9 packet
            if (!plr->isVisibleObject(obj->getGuid()))
            {
                ByteBuffer buf;
    ...
    for (auto it = farSet.begin(); it != farSet.end(); )
    {
        ...
        if (!stillInside)
        {
            if (obj && plr->isVisibleObject(obj->getGuid()) && !plr->canSee(obj))
                plr->getUpdateMgr().pushOutOfRangeGuid(obj->GetNewGUID());
...
Summon* WorldMap::summonCreature(uint32_t entry, LocationVector pos, WDB::Structures::SummonPropertiesEntry const* properties /*= nullptr*/, uint32_t duration /*= 0*/, Object* summoner /*= nullptr*/, uint32_t spellId /*= 0*/)
{
    ...
    factory_->attachToWorld(summon, summon->GetPosition(), false, false);
            ...
            else
            {
                if (pObj->IsInWorld())
                {
                    ...
                    if (count)
                    {
                        ...
                        getVisibilitySystem().collectViewersOf(pObj->GetNewGUID(), s_guids);

                        for (const WoWGuid& guid : s_guids)
                        {
                            Player* player = getPlayer2(guid);
                            if (!player)
                                continue;

                            // Make sure that the target player can see us.
                            if (player && player->seesGuid(pObj->GetNewGUID()))
                            {
                                const uint64_t viewerRaw = player->GetNewGUID().getRawGuid();
                                ...
                                pObj->BuildValuesUpdateBlockForPlayer(&update, player);
                                player->getUpdateMgr().pushUpdateData(&update, count);
                                update.clear();
                            }
                        }
                    }
