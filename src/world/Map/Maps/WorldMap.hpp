...
public:
    ...
    Creature* getCreature2(const uint32_t& low) { return registry_ ? registry_->getCreature(low) : nullptr; }
    GameObject* getGameObject2(const WoWGuid& g) { return registry_ ? registry_->getGameObject(g) : nullptr; }
    GameObject* getGameObject2(const uint32_t& low) { return registry_ ? registry_->getGameObject(low) : nullptr; }
    ...
    Unit* getUnit2(const uint32_t lowGuid) const;
