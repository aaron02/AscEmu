...
namespace visibility
{
    ...
    public:
        ...
        bool release(ObjectHandle h) { return pool_.release(h); }
        ...
        std::uint32_t liveCount() const { return pool_.liveCount(); }
        std::uint32_t poolCapacity() const { return pool_.capacity(); }

        void shrinkObjectPool(std::size_t minCapacity = 64, std::size_t maxFreeRatio = 4)
        {
            pool_.shrinkToFit(minCapacity, maxFreeRatio);
        }
