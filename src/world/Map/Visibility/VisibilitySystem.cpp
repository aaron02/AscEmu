...
namespace visibility
{
    ...
    void VisibilitySystem::collectVisibleObjectsForViewer(const WoWGuid& viewer, std::vector<WoWGuid>& out) const
    {
        ...
        auto it = visibleNow_.find(vraw);
        if (it == visibleNow_.end())
            return;
    ...
    void VisibilitySystem::clearVisibleObjectsForViewer(const WoWGuid& viewer, std::vector<WoWGuid>& out)
    {
        ...
        for (uint64_t raw : it->second)
        {
            ...
            if (auto jt = seenBy_.find(raw); jt != seenBy_.end())
            {
    ...
    void VisibilitySystem::resetViewerForRelocation(ObjectHandle who, std::vector<WoWGuid>& out)
    {
        ...
        // Remove stale subscriptions synchronously. Do not rely on CmdClearRole
    ...
    InterestProfile VisibilitySystem::buildInterestProfile(Object* obj) const
    {
        ...
        if (obj->isGameObject() && (obj->ToGameObject()->getGoType() == GAMEOBJECT_TYPE_TRANSPORT || obj->ToGameObject()->getGoType() == GAMEOBJECT_TYPE_MO_TRANSPORT || obj->ToGameObject()->getGoType() == GAMEOBJECT_TYPE_DESTRUCTIBLE_BUILDING))
        {
            profile.publishMode = PublishMode::GridWide;
            profile.publishCells = 0;
            profile.publishPlayersOnly = true;
...
////////////////////////////////////////////////////////////////////////////////////////
    ...
    void VisibilitySystem::refreshObjectVisibility(ObjectHandle objH)
    {
        ...
        if (!spatialIndex_.tryGet(objH, slot) || !slot->native)
            return;

        // 1) Re-evaluate how all candidate viewers see this object. This replaces
        // the old Unit::updateVisibility() branch that directly created/destroyed
        // this unit for nearby players. collectCandidateViewersForObject() also
        // covers published objects, not just viewers subscribed to the object's cell.
        proximityAffectViewersForObject(objH);

        // 2) If this object itself is a viewer, re-evaluate what it can see. This
        // replaces the old player branch of Unit::updateVisibility() and keeps
        // canSee() driven state changes, spirit/corpse visibility, GM invis, etc.
        // inside the new VisibilitySystem state machine.
