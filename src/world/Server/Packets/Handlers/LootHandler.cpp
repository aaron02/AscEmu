    ...
    else
    {
        if (Creature* creature = GetPlayer()->getWorldMapCreature(lguid.getRawGuid()))
        {
            ...
            if (creature->loot.isLooted())
            {
                ...
                s_guids.clear();
                s_guids.reserve(64);
                creature->getWorldMap()->getVisibilitySystem().collectViewersOf(creature->GetNewGUID(), s_guids);

                // Make creature no Longer Lootable we have no more loot left
                for (const WoWGuid& id : s_guids)
                {
                    Player* player = creature->getWorldMap()->getRegistry().getPlayer(id);
                    if (!player)
                        continue;

                    if (creature->isTaggedByPlayerOrItsGroup(player))
                    {
#if VERSION_STRING < Mop
                        creature->BuildFieldUpdatePacket(player, getOffsetForStructuredField(WoWUnit, dynamic_flags), 0);
#else
                        creature->BuildFieldUpdatePacket(player, getOffsetForStructuredField(WoWObject, dynamic_field), 0);
#endif
                    }
                }
            ...
            else
            {
                ...
                for (const WoWGuid& id : s_guids)
                {
                    ...
                    if (creature->isTaggedByPlayerOrItsGroup(player))
                    {
                        player->sendLootUpdate(creature);
                    }
